...
# ---- simple local click store (works on Render disk, optional) ----
...
def create_track_link(payload: TrackCreateIn):
    ...
    qs = []
    if payload.campaign: qs.append(("utm_campaign", payload.campaign))
    if payload.source: qs.append(("utm_source", payload.source))
    if payload.medium: qs.append(("utm_medium", payload.medium))
    if qs:
    ...
    # Render sets the service URL; frontend can also build this itself
...
def normalize_feed(payload: NormalizeIn, x_admin_api_key: Optional[str] = Header(default=None)):
    ...
    def norm(u: str) -> str:
        ...
        # TikTok: strip query params, keep main url
        if "tiktok.com" in u:
            u = u.split("?")[0]
            return u

        # Instagram: keep base URL
        if "instagram.com" in u:
            u = u.split("?")[0]
            return u

        # Facebook: keep base URL
    ...
    out = [norm(u) for u in payload.urls]
    return {"urls": out}
...
async def stripe_webhook(request: Request):
    ...
    # Minimal handling (extend later)
    etype = event["type"]
    # You can log or branch here:
    # - checkout.session.completed
    # - invoice.paid
    # - customer.subscription.updated
    # - customer.subscription.deleted
    return {"ok": True, "type": etype}
'@ | Set-Content -Encoding UTF8 .\backend\main.py
